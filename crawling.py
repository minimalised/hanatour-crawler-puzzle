...
# ==========================================
# [함수 2] ⚡ 최종 디테일 업그레이드: 35~45자 완전 보장형 단일 LLM 생성기
# ==========================================
async def generate_naver_titles_llm(p, semaphore):
    ...
    async with semaphore:
        ...
        grade_rule = ""
        if price_grade == "세이브":
            grade_rule = "- 현재 상품 등급: [세이브] ➡️ '세이브' 단어는 영문/한글 모두 사용 금지. 대신 [실속형], [가성비추천], [합리적플랜], [부담없는플랜] 중 하나를 반드시 문맥에 녹여 가성비를 극대화할 것."
        elif price_grade == "스탠다드":
            grade_rule = "- 현재 상품 등급: [스탠다드] ➡️ '스탠다드' 단어는 영문/한글 모두 사용 금지. 대신 [핵심일정], [완벽구성], [알찬여행], [밸런스추천] 중 하나를 반드시 문맥에 녹여 알찬 스펙을 강조할 것."
        elif price_grade == "프리미엄":
            grade_rule = "- 현재 상품 등급: [프리미엄] ➡️ '프리미엄' 단어는 영문/한글 모두 사용 금지. 대신 [노쇼핑], [노팁], [노옵션], [자유시간포함], [전일정5성숙소] 중 최소 1~2개 이상을 전면에 노출하여 프리미엄 혜택을 강조할 것."
...
- 여행 지역: {p['지역']}
- 가격/금액: {p['가격']:,}원
- 필수 출발지 문구: {departure} (★주의: 이 문구가 존재할 경우 반드시 최종 상품명 가장 맨 앞에 강제 위치해야 함)
{grade_rule}

[⚠️ ★★★ 중요: 글자 수 강제 확보 및 유연한 자율 지침 ★★★]
현재 생성되는 상품명들이 15~20자 내외로 지나치게 짧아 네이버 쇼핑 검색 노출 가중치에서 심각한 불이익을 받고 있습니다. 
단어를 과도하게 아끼지 말고, 모든 상품명은 반드시 '공백을 포함하여 최소 35자 ~ 최대 45자 사이'의 묵직한 길이로 꽉 채워 출력하세요. (50자 절대 초과 금지)

이를 위해 아래의 [5단계 명사구 컴포넌트 구조]를 유기적으로 조합하여 글자 수를 여유 있게 확보해야 합니다.
- 결합 공식: [출발지(있는 경우)] + [지역명 및 일정 정보] + [등급 치환 키워드] + [원본 상품명 속 핵심 혜택/USP 단어 2개 이상 가공 결합] + [콘셉트별 타겟 및 소구 수식어 명사 조합]
- 정석 길이 예시 타겟팅: "[인천출발] 방콕 파타야 5일 핵심일정 산호섬투어 디너크루즈포함 부모님 효도여행 추천" (공백 포함 44자 - 매우 이상적)

[❌ 전 콘셉트 공통 금지 가이드라인]
1. 글자 수 하한선 엄수: 공백 포함 35자 미만으로 대충 짧게 끊어 내뱉는 행위를 절대 엄금한다. 무조건 35자~45자 사이의 풍성한 키워드로 가득 채울 것.
2. 유연한 중복 제약: 지역명(방콕, 파타야) 및 필수 결합 명사, 조사를 제외한 일반 단어의 '무의미하고 연속적인 기계적 반복'만 금지한다. 단어를 지나치게 검열하느라 문장 길이를 축소시키지 말 것.
3. 해시태그 날것 노출 금지: '#' 기호나 해시태그 형태를 그대로 노출하지 말고 마케팅 조사와 결합해 명사구로 가공할 것. (ex: #디너크루즈 -> 디너크루즈포함, #5성호텔 -> 5성급호텔숙박완비)
4. 정제성: '신상품', '특가', '대박', '★' 같은 홍보성 성격의 특수문자나 유치한 단어는 절대 포함하지 않는다.
5. 결과물 간 상호 중복 엄금: 생성되는 15개의 상품명은 한 상품 안에서 조사나 어순만 바꾼 수준이 아니라 완전히 다른 키워드 조합과 배열을 가져야 한다.
...
■ 콘셉트 D (감성/트렌디형 - 3개): 요즘뜨는핫플, 인생샷명소 탐방, 감성힐링스팟투어 등 트렌디 단어가 겹치지 않게 분산.
