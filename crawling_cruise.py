...
async def run_crawler():
    ...
    # [최신 표준 안정화] 환경 변수에서 JSON 텍스트를 직접 읽어 메모리에서 바로 인증 (디스크에 파일 생성 X)
    json_raw = os.environ.get("GOOGLE_JSON_RAW")
    
    try:
        if json_raw:
            # GitHub Actions 환경: 환경 변수의 문자열을 딕셔너리로 파싱하여 파일 없이 직접 인증
            service_account_info = json.loads(json_raw)
            creds = Credentials.from_service_account_info(service_account_info, scopes=scopes)
        else:
            # 로컬 개발 환경: 환경 변수가 없을 때는 기존처럼 프로젝트 내 secrets.json 파일 참조
            ...
            try:
                ...
                for spreadsheet_id in target_spreadsheet_ids:
                    try:
                        ...
                        # [최신 표준 안정화] 범위 명시 및 명확한 키워드 인자 전달 구조로 업그레이드
